...
describe('The total types for collection observable service', () => {

  let
    CollectionTypesObservable,
    subscribeCalled,
    onNextCalled;
  ...
  beforeEach(inject((_rxSubject_,
                     _CollectionTypesObservable_) => {

    CollectionTypesObservable = _CollectionTypesObservable_;
  ...
  beforeEach(() => {
    ...
    spyOn(CollectionTypesObservable, 'set').and.callThrough();
    spyOn(CollectionTypesObservable, 'subscribe').and.callThrough();

  });

  it('should update the observers and get the new value.', () => {

    CollectionTypesObservable.subscribe();
    expect(subscribeCalled).toBe(true);
    CollectionTypesObservable.set(2);
    expect(onNextCalled).toBe(true);
    // This will have been updated in the call through.
    let testValue = CollectionTypesObservable.get();
