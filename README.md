...
Tagger offers a way to publish information about the digital work produced by a community or organization. Information about a digital work or digital collection is added to a Tagger `Collection` object.  The `Collection` is assigned to a general `Area` that can be topical or administrative (e.g. student research or University Archives). Areas in turn can have one or more `Collection Groups` that are the specific  home of the work (for example, a specific collection within an archive).  Subject tags and content types can also be assigned to support browsing and discovery. 
